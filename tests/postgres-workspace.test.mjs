...
test("PostgreSQL routes enforce tenant scope and persist staff and intake audits atomically", async () => {
  ...
  try {
    ...
    assert.equal((await request(assignmentPath, le, "PATCH", { ...reassignment, chemistId: null, expectedChemistId: replacement.id })).status, 200);
    ...
    const printPath = `/api/v1/bullion/samples/${first.id}/print`;
    exam.silverResult = 193.35;
    exam.measurementEntries = [1500, 250, 100, 650].map((reading, index) => ({ label: String(index), reading }));
    assert.equal((await request(printPath, replacement, "POST", {})).status, 409);
    ...
    assert.equal(chemistAfterSubmit.find((sample) => sample.id === first.id).status, "submitted");
    assert.equal((await request("/api/v1/bullion/examinations", replacement, "POST", { ...exam, bullionItemId: first.id, expectedRevision: 1 })).status, 409);
    ...
    assert.equal((await request(printPath, oldActor, "POST", {})).status, 404);
    assert.equal((await request(printPath, foreign, "POST", {})).status, 403);
    assert.equal((await request(printPath, le, "POST", {})).status, 403);
    assert.equal((await request("/api/v1/bullion/examinations", le, "POST", { ...exam, bullionItemId: first.id })).status, 403);
    assert.equal((await request(printPath, melter, "POST", {})).status, 403);
    assert.equal((await request(printPath, replacement, "POST", { chemistId: foreign.id })).status, 400);
    assert.equal((await request(printPath, replacement, "POST", {})).status, 409);
    ...
    const printed = await request(printPath, replacement, "POST", { chemistId: oldActor.id });
    assert.equal(printed.status, 200);
    assert.match(printed.headers.get("cache-control"), /no-store/);
    const printReport = (await printed.json()).data;
    assert(printReport.customerName);
    assert.equal(printReport.centerType, "private_assay_center");
    assert.equal(printReport.managerName, "Test lab_manager");
    ...
    assert.deepEqual(printReport.entries.map(entry => entry.bullionNo), eightBatch.items.map(item => item.bullionNo));
    assert.equal(printReport.sample.status, "submitted");
    assert.equal(printReport.sample.analysisNo, String(storedNumber));
    const printAudit = (await database.query("SELECT new_values FROM audit_logs WHERE action = 'bullion_examination.print_requested' AND entity_id = $1", [first.id])).rows[0].new_values;
    assert.equal(printAudit.printedByUserId, replacement.id);
    assert.equal(printAudit.selectedChemistId, oldActor.id);
    assert.equal((await request(printPath, replacement, "POST", {})).status, 200);
    const otherRow = remainingSamples[0];
    const otherReport = (await (await request(`/api/v1/bullion/samples/${otherRow.id}/print`, chemists.find(actor => actor.id === otherRow.assigned_chemist_id), "POST", {})).json()).data;
    assert.equal(otherReport.certificateNo, printReport.certificateNo);
    assert.equal(otherReport.issuedAt, printReport.issuedAt);
    const afterReprint = (await (await request("/api/v1/bullion/samples", le)).json()).data.find(sample => sample.id === first.id);
    assert.equal(afterReprint.completedAt, completedSample.completedAt);
    assert.deepEqual(otherReport.entries, printReport.entries);
    ...
    const updatedPrint = (await (await request(printPath, replacement, "POST", {})).json()).data;
    ...
    for (const [prefix, type, expected] of [[null, "government_assay_center", "0001"], ["22", "government_assay_center", "220001"], ["27", "government_assay_center", "270001"], [null, "private_assay_center", "550001"]]) {
      ...
      async function completeRequest() {
        ...
        return `/api/v1/bullion/samples/${id}/print`;
      }
      const historicalPath = await completeRequest();
      const historicalItem = historicalPath.split('/')[5];
      ...
      await database.query("INSERT INTO bullion_certificates (batch_id, assay_center_id, issue_year, sequence_no, entries, issued_at) SELECT batch_id, $2::uuid, $3::int, 924, '[]'::jsonb, make_timestamptz($3::int, 12, 31, 15, 59, 59, 'UTC') FROM bullion_intake_items WHERE id = $1", [historicalItem, organization.id, year - 1]);
      const historical = (await (await request(historicalPath, analyst, "POST", {})).json()).data;
      assert.equal(historical.certificateNo, "0924");
      assert.equal(historical.certificateYear, year - 1);
      const path = await completeRequest();
      const response = await request(path, analyst, "POST", {});
      assert.equal(response.status, 200);
      const certificate = (await response.json()).data;
      assert.equal(certificate.certificateNo, "0001");
      assert.equal(certificate.certificateYear, year);
      assert.equal(certificate.entries[0].bullionNo, expected.slice(0, -1) + "2");
      const repeated = (await (await request(path, analyst, "POST", {})).json()).data;
      assert.equal(repeated.certificateNo, "0001");
      assert.equal(repeated.issuedAt, certificate.issuedAt);
      assert.equal((await request(path, replacement, "POST", {})).status, 404);
      const next = (await (await request(await completeRequest(), analyst, "POST", {})).json()).data;
